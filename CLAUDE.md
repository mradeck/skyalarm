...
**Aktuell:** v26.08.47.4 — **CARTO-Basemap-Ersatz (Dark-Karten-API-Fix).** CARTO (`basemaps.cartocdn.com`) liefert ohne API-Key nur noch „API KEY REQUIRED"-Wasserzeichen-Kacheln (betraf die dunkle Kartenansicht). Fix analog zu SkyCheck v116.1: „Dark" = OpenStreetMap-Tiles per CSS-Filter invertiert (`.tiles-dark`, kein Key), „Hell" (CARTO light) ersatzlos entfernt (OSM ist bereits hell); `className: dark ? 'tiles-dark'` an beiden Basemap-`tileLayer`-Stellen (`avInitMap`/`setMapStyle`), Theme-Kopplung `lightMode?1:0` unverändert, CARTO aus CSP und Quellen-Chips entfernt. Vorgänger v26.08.47.3 — **Impressum-Link im Info-Modal.** Die „i"-Info-Kachel (`#info-modal`, `.modal-srcs`) verlinkt jetzt zusätzlich das Impressum (`multikopterschule.de/Kontakt-Impressum/IMPRESSUM/`), analog zur SkyCheck-Quellenleiste. Vorgänger v26.08.47.2 — **Wetter-Fetch gehärtet:** BrightSky-Abruf mit 6-s-`AbortController`-Timeout (passend zum ADS-B-Poll); ein hängender Abruf blockiert den 10-min-Wetter-Poll nicht mehr (`updateWeatherOverlay` behält den letzten guten Datensatz). Vorgänger v26.08.47.1 — Die Startseite verlinkt GPS2UTM, SkyCheck und PointCloud Manager; SkyAlarm folgt dem PointCloud-Schema `vYY.MM.major.subversion`.
...
| **Dark-Basemap** | `tile.openstreetmap.org` (CSS-invertiert) | – | Dunkle Kartenansicht = OSM-Tiles per `.tiles-dark`-Filter invertiert (seit v26.08.47.4; ersetzt CARTO, das ohne API-Key nur noch Wasserzeichen-Kacheln liefert) |
...
| Version | Änderungen |
|---|---|
| v26.08.47.4 | **CARTO-Basemap-Ersatz (Dark-Karten-API-Fix).** CARTO-Basemaps (`{s}.basemaps.cartocdn.com/dark_all` & `light_all`) liefern ohne API-Key nur noch „API KEY REQUIRED"-Wasserzeichen-Kacheln — die dunkle Kartenansicht war unbrauchbar (v. a. mobil). Portierung des SkyCheck-v116.1-Fixes: (a) neue CSS-Regel `.tiles-dark { filter: invert(1) hue-rotate(180deg) brightness(.95) contrast(.9) saturate(.7); }`. (b) `AV.STYLES`: „Dark" nutzt jetzt OSM-Tiles (`{s}.tile.openstreetmap.org`, `s:'abc'`, `dark:true`), „Hell" (CARTO light) ersatzlos entfernt — OSM (Index 1) ist bereits die helle Karte, die Theme-Kopplung `setMapStyle(lightMode ? 1 : 0)` bleibt gültig (1=OSM hell, 0=Dark=invertiertes OSM). (c) `className: style.dark ? 'tiles-dark' : ''` an beiden Basemap-`L.tileLayer`-Stellen (`avInitMap`, `setMapStyle`). (d) CARTO aus `netlify.toml`-CSP (`img-src`) und aus beiden Quellen-Chip-Listen (Splash + Info-Modal) entfernt. Verifiziert: Tiles kommen von `tile.openstreetmap.org`, kein CARTO, `.tiles-dark`-Filter aktiv. |
| v26.08.47.3 | **Impressum im Info-Modal.** Die „i"-Info-Kachel (`#info-modal` → `.modal-srcs`) enthält nun einen Impressum-Link (`https://www.multikopterschule.de/Kontakt-Impressum/IMPRESSUM/`, bei den persönlichen Links vor `michael-radeck.de`) — angeglichen an die SkyCheck-Quellenleiste. |
