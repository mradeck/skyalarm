...
**Aktuell:** v0.37 — Umsetzung der Code-Review-Befunde (v0.32, siehe `docs/code-review-2026-07-16.md`) in fünf Releases: v0.33 Alarm-Zustellbarkeit (Screen Wake Lock, Audio-Unlock bei User-Geste, pulsierender visueller Alarm-Rahmen), v0.34 Höhenbezug AGL statt MSL (Geländehöhe via Open-Meteo-Elevation-API) samt Ausschluss von Bodenverkehr/unbekannter Höhe vom Alarm, v0.35 XSS-Escaping aller Fremddaten-HTML-Sinks + CSP-Header, v0.36 Alarm-Hysterese (Hold-Down gegen Beep-Flattern) und In-Flight-Guard/AbortController-Timeout im Poll, v0.37 Wetter-Overlay hält letzten guten Wert bei Fehler + Alarm-Banner zeigt nächstgelegenes Target
...
| v0.37 | Code-Review-Fix N1/N3. **N1**: `updateWeatherOverlay` behält bei fehlgeschlagenem BrightSky-Abruf den letzten guten Datensatz (`if (w) AV.lastWeather = w`), statt das Eis-/Nebel-/Wind-Overlay bis zum nächsten 10-min-Intervall zu leeren. **N3**: Alarm-Banner/Beep wählen das **nächstgelegene** Target statt `near[0]` (Array-Insert-Reihenfolge). Offen bleiben laut Review: M1 (Netz-Backoff), M2 (Marker-Vollrebuild-Churn), M3 (offener OGN-Proxy ohne Rate-Limit), N2 (Pseudo-Hex-Kollision), N4 (Vereisungsband bis −10 °C) |
