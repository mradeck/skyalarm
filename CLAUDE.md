...
**Aktuell:** v0.31 — Auto-Start-Funktion: Eine persistente Einstellung (localStorage `skyalarm-autostart`) überspringt beim App-Start den Splash-Button „Standort verwenden" und springt direkt in die Karte, sofern die Geolokalisierungs-Freigabe bereits erteilt ist. Bedienbar über eine Checkbox auf dem Splash-Screen und einen Toggle (`#av-autostart`, ⚡) in der Kartensteuerleiste. Permission-Gate via `navigator.permissions.query` verhindert Sackgassen bei nicht erteilter Freigabe (insb. iOS/Safari ohne User-Geste)
