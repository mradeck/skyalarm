...
**Aktuell:** v0.32 — Bugfix Wetterdaten-Staleness: `fetchWeather` baute die BrightSky-URL mit Datum-only-Werten (`date=YYYY-MM-DD&last_date=YYYY-MM-DD`); lagen `now` und `now+6h` am selben UTC-Kalendertag (also vor 18:00 UTC — fast den ganzen Tag), war `last_date == date` und BrightSky lieferte NUR den 00:00-UTC-Datensatz. Sämtliche Wetteranzeigen (Wind, Böen, Vereisung, Eis-/Nebelalarm) zeigten damit ganztägig Mitternachtswerte. Fix: volle ISO-8601-Timestamps (`now−1h` bis `now+6h`) in der Abfrage
