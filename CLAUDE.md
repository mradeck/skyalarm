...
**Aktuell:** v0.30 — Robuster DiPUL-Layer-Recovery: Ein serverseitig defekter GeoServer-Layer (`temporaere_betriebseinschraenkungen`, dessen SQL-View die umbenannte Spalte `internal_reference` statt `external_reference` referenziert) ließ wegen GeoServers Alles-oder-nichts-Rendering die gesamte kombinierte WMS-Anfrage (GetFeatureInfo UND GetMap) mit ServiceException scheitern — es erschienen gar keine Zonen mehr. Fix analog SkyCheck v0.77: defekte Layer werden zur Laufzeit abgefangen statt fest entfernt
